...
setwd("~/Desktop/San Andrés/2022/2do trimestre/Herramientas computacionales para investigación/Tarea 5/videos 2 y 3 2022/videos 2 y 3/data")

library(rgdal)
lnd <- readOGR(dsn = "~/Desktop/San Andrés/2022/2do trimestre/Herramientas computacionales para investigación/Tarea 5/videos 2 y 3 2022/videos 2 y 3/data/london_sport.shp")
...
lnd <- readOGR("~/Desktop/San Andrés/2022/2do trimestre/Herramientas computacionales para investigación/Tarea 5/videos 2 y 3 2022/videos 2 y 3/data/london_sport.shp")
...
crime_data <- read.csv("C:/Users/estef/Desktop/San Andrés/2022/2do trimestre/Herramientas computacionales para investigación/Tarea 5/videos 2 y 3 2022/videos 2 y 3/data/mps-recordedcrime-borough.csv", stringsAsFactors = FALSE)
...
# Usamos left_join para quedarnos con la data que coincide
...
head(left_join(lnd@data, crime_ag)) # como no coinciden los nombres, you will need "by"
lnd@data <- left_join(lnd@data, crime_ag, by = c('name' = 'Borough')) # Tienen distinto nombre, usamos esta
 
# tmap was created to overcome some of the limitations of base graphics and ggmap.
library(tmap) # load tmap package 
lnd@data = transform(lnd@data, cripop = CrimeCount/ as.numeric(Pop_2001))
qtm(lnd, "cripop") # plot the basic map
qtm(shp = lnd, fill = "cripop", fill.title = "Cantidad de robos\n per cápita")
...
map <- ggplot(lnd_f, aes(long, lat, group = group, fill = cripop)) +
  geom_polygon() + coord_equal() +
  labs(x = "Este (m)", y = "Norte (m)",
       fill = "Cantidad de\nrobos per cápita") +
  ggtitle("Cantidad de robos per cápita")
...
london_data <- read.csv("C:/Users/estef/Desktop/San Andrés/2022/2do trimestre/Herramientas computacionales para investigación/Tarea 5/videos 2 y 3 2022/videos 2 y 3/data/census-historic-population-borough.csv")
...
# Rename the date variable, reemplaza Pop_ por vacío
